...
(function ($) {
  ...
  var API = {
    ...
    getFormatTime: function (_this) {
      return _this.onlyTime
        ? _this.config.format
        : _this.config.format.split(" ")[1];
    },
    // 时分秒格式正则
    ...
    dayReg: function (_this) {
      ...
      var regText = format
        .replace(/YYYY/, "([1-9]{1}[0-9]{3})")
        .replace(splitStrReg, "(" + _this.splitStr + "?)")
    ...
    newDateFixed: function (_this, temp) {
      ...
      var str =
        isIE || (isSafari && _this.config.format !== "YYYY-MM") ? "/" : "-";
      var reg = new RegExp(_this.splitStr, "g");
      var result = !temp
        ? new Date()
        : _this.splitStr
        ? new Date(temp.replace(reg, str))
        : new Date(temp);
      return result;
    },
    // 范围获取具体年月日
    ...
    minMaxFill: function (_this, result, index, type) {
      ...
      if (type === "month") {
        ...
        val =
          result.year +
          _this.splitStr +
          API.fillTime(result.month) +
          _this.splitStr +
          API.fillTime(result.day);
      }
      if (_this.hasTime) {
        val +=
          " " +
          _this.$container.find(".c-datePicker__input-time").eq(index).val();
      }
    ...
    judgeTimeRange: function (_this, $day, $time, index) {
      ...
      var isBefore = _this.config.min
        ? _moment.isBefore(API.newDateFixed(_this, _this.config.min))
        : false;
      var isAfter = _this.config.max
        ? _moment.isAfter(API.newDateFixed(_this, _this.config.max))
        : false;
      if (!isBefore && !isAfter) {
        return;
      }
    ...
    timeVal: function (_this, type) {
      var timeFormat = _this.onlyTime
        ? _this.config.format
        : _this.config.format.split(" ")[1];
      return type === "min"
        ? timeFormat.replace(/HH/, "00").replace(/mm/, "00").replace(/ss/, "00")
        : timeFormat
            .replace(/HH/, "23")
            .replace(/mm/, "59")
            .replace(/ss/, "59");
    ...
    getOnlyTimeMinMax: function (_this) {
      ...
      var hasMinMax =
        hasMax && hasMin ? true : hasMax ? "max" : hasMin ? "min" : false;
      if (hasMin) {
        var _min = min.split(":");
    ...
    countSecond: function (result) {
      return result.length === 2
        ? (result = Number(result[0]) * 60 + Number(result[1]))
        : result.length === 3
        ? (result =
            Number(result[0]) * 3600 +
            Number(result[1]) * 60 +
            Number(result[2]))
        : false;
    },
  ...
  var RENDERAPI = {
    ...
    timeLiTpl: function (className, time) {
      var html =
        '<li class="c-datepicker-time-spinner__item ' +
        className +
        '">' +
        time +
        "</li>";
    ...
    timeTpl: function (className, li) {
      var html =
        '<div class="c-datepicker-time-spinner__wrapper c-datepicker-scrollbar">' +
        '<div class="c-datepicker-scrollbar__wrap ' +
        className +
    ...
    timeMainTpl: function (nameOptions, time) {
      var html =
        '<div class="c-datepicker-time-panel c-datepicker-popper" style="">' +
        ...
        '<button type="button" class="c-datepicker-time-panel__btn min">' +
        nameOptions.zero +
        "</button>" +
        '<button type="button" class="c-datepicker-time-panel__btn max">23:59</button>' +
        '<button type="button" class="c-datepicker-time-panel__btn cancel">' +
        nameOptions.cancel +
        "</button>" +
        '<button type="button" class="c-datepicker-time-panel__btn confirm">' +
        nameOptions.confirm +
        "</button>" +
        "</div>" +
        "</div>";
    ...
    sideBarTpl: function (button) {
      var html =
        '<div class="c-datepicker-picker__sidebar">' + button + "</div>";
      return html;
    },
    pickerFooterTpl: function (nameOptions, className, text) {
      ...
      var html =
        ...
        clearHtml +
        '<button type="button" class="c-datepicker-button c-datepicker-picker__link-btn c-datepicker-button--text c-datepicker-button--mini ' +
        className +
        '">' +
        "<span>" +
        text +
    ...
    pickerHeaderPrevTpl: function (nameOptions, className) {
      var html =
        ...
        nameOptions.prevYear +
        '"></i>' +
        '<i class="kxiconfont icon-left c-datepicker-picker__icon-btn ' +
        className +
        '__prev-btn month" aria-label="' +
        nameOptions.nextMonth +
        '"></i>';
      return html;
    },
    ...
    pickerTimeHeaderTpl: function (nameOptions, className) {
      var html =
        ...
        nameOptions.chooseDay +
        '" class="c-datepicker-input__inner c-datePicker__input-day">' +
        "</div>" +
        "</span>" +
        '<span class="' +
        className +
        ...
        '<div class="c-datepicker-input c-datepicker-input--small">' +
        '<input type="text" autocomplete="off" placeholder="' +
        nameOptions.chooseTime +
        '" class="c-datepicker-input__inner c-datePicker__input-time">' +
        "</div>" +
        "</span>";
      return html;
    },
    pickerOnlyTimeHeaderTpl: function (className, name) {
      var html =
        ...
        className +
        '__editor-wrap">' +
        '<div class="c-datepicker-only-time-title">' +
        name +
        "</div>" +
        "</span>";
      return html;
    },
    rangePickerMainTpl: function (
      nameOptions,
      hasTime,
      hasSidebar,
      yearEnd,
      ...
      var pickerHeader = RENDERAPI.pickerHeaderTpl(
        nameOptions,
        className,
        prev,
        "",
      ...
      var pickerHeader2 = RENDERAPI.pickerHeaderTpl(
        nameOptions,
        className,
        "",
        next,
      ...
      var footer = RENDERAPI.pickerFooterTpl(
        nameOptions,
    ...
    rangePickerMainOnlyTimeTpl: function (nameOptions, hasTime) {
      var className = "c-datepicker-date-range-picker";
      var headerBegin = RENDERAPI.pickerOnlyTimeHeaderTpl(
        className,
        nameOptions.begin
      );
      var headerEnd = RENDERAPI.pickerOnlyTimeHeaderTpl(
        className,
        nameOptions.end
      );
    ...
    pickerFooterNowButton: function (nameOptions) {
      var html = RENDERAPI.pickerFooterTpl(
        nameOptions,
    ...
    datePickerMainTpl: function (nameOptions) {
      ...
      // 单个
      var html =
        '<div class="c-datepicker-picker c-datepicker-date-picker c-datepicker-popper {{hasTime}} {{hasSidebar}}" x-placement="top-start">' +
        '<div class="c-datepicker-picker__body-wrapper">' +
        "{{sidebar}}" +
        '<div class="c-datepicker-picker__body">' +
    ...
    datePickerMainOnlyTimeTpl: function (hasTime) {
      var header = RENDERAPI.pickerOnlyTimeHeaderTpl(
        "c-datepicker-date-picker",
        ""
      );
      var arrow = RENDERAPI.pickerArrowTpl();
      // 单个-只有时分秒
  ...
  $.extend(Year.prototype, {
    ...
    renderHtml: function (year) {
      ...
      var activeYear = val
        ? API.getTimeFormat(moment(API.newDateFixed(this.picker, val))).year
        : false;
      var nameOptions = $.fn.datePicker.dates[this.picker.language];
      this.picker.$container
        .find(".c-datepicker-date-picker__header-year span")
        .text(min + nameOptions.headerYearLink + "-" + (min + 9));
      for (var index = 0; index < 10; index++) {
        var _val = min + index;
        var className = _val == activeYear ? "current available" : "available";
        if (
          _val < this.picker.minJson.year ||
          _val > this.picker.maxJson.year
        ) {
          className += " disabled";
        }
  ...
  $.extend(Month.prototype, {
      ...
      if (!this.picker.config.isRange) {
        ...
        this.picker.$container.on(
          ...
          function () {
            ...
            var year = _this.picker.$container
              .find(".c-datepicker-date-picker__header-year span")
              .text();
            var month =
              _this.picker.$container
    ...
    renderHtml: function () {
      ...
      var temp = "";
      var html = "";
      var nowYear = this.picker.$container
        .find(".c-datepicker-date-picker__header-year span")
        .text();
      ...
      var val = this.picker.$input.val();
      var formatResult = API.getTimeFormat(
        moment(API.newDateFixed(this.picker, val))
      );
      var activeMonth =
        val && formatResult.year == nowYear ? formatResult.month : false;
      var nameOptions = $.fn.datePicker.dates[this.picker.language];
      ...
      for (var index = 0; index < 12; index++) {
        ...
        var className =
          _val === activeMonth ? "current available" : "available";
  ...
  $.extend(Day.prototype, {
    ...
    eventSingle: function () {
      ...
      function clickDate(_this, $target) {
        ...
        var val = _this.picker.$container
        ...
        if (!val) {
          ...
          setValue.call(
            _this,
            activeNum,
            moment(API.newDateFixed(_this.picker, val)),
            moment(API.newDateFixed(_this.picker, inputVal))
          );
        }
        if (!_this.picker.hasTime) {
          ...
          API.judgeTimeRange(
            _this.picker,
            _this.picker.$container.find(".c-datePicker__input-day"),
            _this.picker.$container.find(".c-datePicker__input-time")
          );
        }
      }
      // 设置选中值
      function setValue(activeNum, input, inputDay) {
        var year = this.picker.$container
          .find(".c-datepicker-date-picker__header-year span")
          .text();
        var month =
          this.picker.$container
    ...
    eventRange: function () {
      ...
      function clickDateRange(_this, $target) {
        ...
        var year = $activeWrap
          .find(".c-datepicker-date-range-picker__header-year span")
          .text();
        var month =
          $activeWrap
            .find(".c-datepicker-date-range-picker__header-month span")
            .text() - 1;
        // 重选
        if (_this.current >= 2) {
        ...
        if (_this.current) {
          ...
          API.judgeTimeRange(
            _this.picker,
            _this.picker.$container.find(".c-datePicker__input-day").eq(index),
            _this.picker.$container.find(".c-datePicker__input-time").eq(index),
            index
          );
        }
      }
      ...
      this.picker.$container.on(
        ...
        function () {
          ...
          var $start = _this.picker.$container
            .find(".c-datePicker__input-day")
            .eq(0);
          var year = $wrap
            .find(".c-datepicker-date-range-picker__header-year span")
          ...
          var start = $start.val();
          var end =
            year + _this.picker.splitStr + month + _this.picker.splitStr + date;
          // 需要交换开始结束时间
          if (
            moment(API.newDateFixed(_this.picker, start)).isAfter(
              API.newDateFixed(_this.picker, end)
            )
          ) {
            var temp = start;
            start = end;
          ...
          _this.addRangeClass(
            moment(API.newDateFixed(_this.picker, start)),
            moment(API.newDateFixed(_this.picker, end)),
            true
          );
        }
      );
    ...
    renderSingle: function (year, month, today, reRender) {
      ...
      if ($year.length && !reRender) {
        ...
        var $content = this.picker.$container.find(
          ".c-datepicker-picker__content"
        );
        // 日历头部
        var $year = this.picker.$container.find(
          ".c-datepicker-date-picker__header-year span"
        );
        var $month = this.picker.$container.find(
    ...
    addCurrentSingle: function () {
      ...
      var result = API.getTimeFormat(
        moment(API.newDateFixed(this.picker, val))
      );
      var year = this.picker.$container
        .find(".c-datepicker-date-picker__header-year span")
        .text();
      var month = this.picker.$container
        .find(".c-datepicker-date-picker__header-month span")
        .text();
      if (result.year == year && result.month == month) {
        var $day = this.picker.$container.find(
          ".c-datepicker-date-table td.available"
        );
    ...
    renderRange: function (year, month, today, baseEnd, reRender) {
      ...
      if ($dateTable.length && !reRender) {
        ...
        // 日历头部
        var $dateTable = this.picker.$container.find(
    ...
    prevNextSingle: function (moveType, type) {
      ...
      var dayFormat = API.getTimeFormat(
        moment(API.newDateFixed(this.picker, day))
      );
      var count = 1;
      if (moveType === "prev") {
        count = -1;
    ...
    renderHtml: function (year, month, activeDay) {
      ...
      var today =
        _moment.month() + 1 === month && _moment.year() === year
          ? _moment.date()
          : "";
      ...
      var weekday = moment()
        .set({ year: year, month: month - 1, date: 1 })
        .weekday();
      var weekdayLast = moment()
      ...
      var nowDate = moment(
        API.newDateFixed(
          this.picker,
          year + this.picker.splitStr + month + this.picker.splitStr + 1
        )
      );
      // 不在范围内
      if (
        (hasMin && nowDate.isBefore(minMonth)) ||
        (hasMax && nowDate.isAfter(maxMonth))
      ) {
        disabledName = " disabled";
      } else if (
        (hasMin && nowDate.isSame(minMonth)) ||
        (hasMax && nowDate.isSame(maxMonth))
      ) {
        isSame = true;
        var minDay, maxDay;
    ...
    addRangeClass: function (defaultStart, defaultEnd, isHover) {
      var $wrap = this.picker.$container.find(
        ".c-datepicker-date-range-picker-panel__wrap"
      );
      ...
      var $years = this.picker.$container.find(
        ".c-datepicker-date-range-picker__header-year"
      );
      var $months = this.picker.$container.find(
        ".c-datepicker-date-range-picker__header-month"
      );
      ...
      var startMoment =
        defaultStart ||
        moment(API.newDateFixed(this.picker, $days.eq(0).val()));
      var endMoment =
        defaultEnd || moment(API.newDateFixed(this.picker, $days.eq(1).val()));
      ...
      var endRangeDate =
        ...
        endMonth +
        this.picker.splitStr +
        API.getMonthDay(endMonth, endYear);
      var isStartBetween = !(
        startMoment.isBefore(API.newDateFixed(this.picker, startRangeDate)) ||
        startMoment.isAfter(API.newDateFixed(this.picker, endRangeDate))
      );
      var isEndBetween = !(
        endMoment.isBefore(API.newDateFixed(this.picker, startRangeDate)) ||
        endMoment.isAfter(API.newDateFixed(this.picker, endRangeDate))
      );
      var index;
      // 所选值不在显示范围内
      var isBefore =
        startMoment.isBefore(API.newDateFixed(this.picker, startRangeDate)) &&
        startMoment.isBefore(API.newDateFixed(this.picker, endRangeDate)) &&
        endMoment.isBefore(API.newDateFixed(this.picker, startRangeDate)) &&
        endMoment.isBefore(API.newDateFixed(this.picker, endRangeDate));
      var isAfter =
        startMoment.isAfter(API.newDateFixed(this.picker, startRangeDate)) &&
        startMoment.isAfter(API.newDateFixed(this.picker, endRangeDate)) &&
        endMoment.isAfter(API.newDateFixed(this.picker, startRangeDate)) &&
        endMoment.isAfter(API.newDateFixed(this.picker, endRangeDate));
      // 当前显示范围在所选范围外
      if (isAfter || isBefore) {
        return;
      ...
      if ($start.is($end)) {
        ...
        if (
          $start
            .parents(".c-datepicker-date-range-picker-panel__wrap")
            .is($end.parents(".c-datepicker-date-range-picker-panel__wrap"))
        ) {
          // 同一行
          if ($startTr.is($endTr)) {
  ...
  $.extend(Time.prototype, {
    ...
    event: function () {
      ...
      this.picker.$container.on(
        ...
        function () {
          ...
          var $time = _this.picker.activeTimeWrap.find(
            ".c-datePicker__input-time"
          );
          var index = _this.picker.$container
            .find(".c-datePicker__input-time")
            .index($time);
          if (!_this.picker.config.isRange) {
            var day = _this.picker.$container
              .find(".c-datePicker__input-day")
              .eq(index)
              .val();
      ...
      this.picker.$container
        ...
        .scroll(function () {
          ...
          timerArr["timer" + index] = setTimeout(
            function () {
              ...
              top = num * 32;
              $(this).scrollTop(top);
              var index = _this.picker.activeTimeWrap
                .find(".c-datepicker-scrollbar__wrap")
                .index($(this));
              var $time = _this.picker.activeTimeWrap.find(
                ".c-datePicker__input-time"
              );
              var day = _this.picker.activeTimeWrap
                .find(".c-datePicker__input-day")
                .val();
              var val = $time.val();
              val = val.split(":");
    ...
    updateTimePanel: function (isShow) {
      var $wrap = this.picker.activeTimeWrap.find(
        ".c-datepicker-scrollbar__wrap"
      );
      var val = this.picker.activeTimeWrap
        .find(".c-datePicker__input-time")
        .val();
      var format = this.picker.config.format.split(" ")[1];
      var regText = format
        .replace(/HH/, "[0-9]{2}")
        .replace(/(mm|ss)/g, "[0-9]{2}");
  ...
  $.extend(OnlyTime.prototype, {
    event: function () {
      ...
      this.picker.$container
        ...
        .scroll(function () {
          ...
          var inputIndex = _this.picker.$container
          ...
          timerArr["timer" + index] = setTimeout(
            function () {
              ...
              var val = TIMEONLYAPI.checkMinMaxGetVal(
                _this.picker,
                valArr,
                inputIndex
              );
              ...
              // 滚动修改对应的最大值最小值
              if (config.isRange) {
                var $otherParent = _this.picker.$container
                  .find(".c-datepicker-date-picker__onlyTime-content")
                  .eq(1 - inputIndex);
                var otherPanelValArr = TIMEONLYAPI.getPanelVal(
                  $otherParent
                ).join(":");
    ...
    updateTimeInput: function (val, $this) {
      if (this.picker.config.isRange) {
        ...
        var index = this.picker.$container
    ...
    renderSingle: function (type) {
      ...
      // 初始化，添加html
      if (!$time.length) {
        var html = DATEPICKERAPI.renderTimePanelHtml(
          this.picker,
          type,
          time1[0],
          time1[1],
    ...
    renderRange: function (type) {
      ...
      this.picker.$inputBegin.val(
        API.getConcatTime(time1[0], time1[1], time1[2])
      );
      this.picker.$inputEnd.val(
        API.getConcatTime(time2[0], time2[1], time2[2])
      );
      var $time = this.picker.$container.find(".c-datepicker-time-panel");
      // 初始化，添加html
      if (!$time.length) {
        var html1 = DATEPICKERAPI.renderTimePanelHtml(
          this.picker,
          type,
          time1[0],
          time1[1],
        ...
        var html2 = DATEPICKERAPI.renderTimePanelHtml(
          this.picker,
          type,
          time2[0],
          time2[1],
    ...
    setMinMaxHour: function (inputIndex) {
      ...
      $panel.each(function (index, _panel) {
        ...
        $wrap
          .find(".c-datepicker-time-spinner__item")
          .each(function (k, _item) {
    ...
    setMinMaxDisabled: function (panelVal, inputIndex) {
      ...
      if (
        (!minJson.hour || val[0] > minJson.hour) &&
        (!maxJson.hour || val[0] < maxJson.hour)
      ) {
        // 分秒在可选范围内
        return;
      }
      if (
        ...
        var $wrapSecondItem = $wrapSecond.find(
          ".c-datepicker-time-spinner__item"
        );
        // 秒都在不可选范围内
  ...
  var DATEPICKERAPI = {
    ...
    initShowObject: function (_this, dataFormat) {
      ...
      if (
        _this.params.format[3] ||
        _this.params.format[4] ||
        _this.params.format[5]
      ) {
    ...
    initParams: function (_this) {
      _this.splitStr = _this.config.format
        .replace(/[YMDhms:\s]/g, "")
        .split("")[0];
      _this.params.format = API.getFormat(_this.config.format);
      // 最大值最小值判断
      _this.minJson = _this.config.min
        ? API.getTimeFormat(moment(API.newDateFixed(_this, _this.config.min)))
        : false;
      _this.maxJson = _this.config.max
        ? API.getTimeFormat(moment(API.newDateFixed(_this, _this.config.max)))
        : false;
    },
    ...
    renderPickerRange: function (target, isBlur) {
      ...
      if (reg.test(val)) {
        ...
        var orderFail =
          index === 0
            ? _moment.isAfter(_momentAnother)
            : _moment.isBefore(_momentAnother);
        // 反过来，需要交换
        if (orderFail) {
          var temp = val;
          val = anotherVal;
  ...
  function SingleDatePicker(datePickerObject) {
    ...
    this.params = {};
    this.language = this.config.language || "es-co";
  ...
  $.extend(SingleDatePicker.prototype, {
    ...
    initShow: function () {
      ...
      var result = inputVal
        ? moment(API.newDateFixed(this, inputVal))
        : moment();
      ...
      if (this.params.isYear || this.params.isMonth) {
        renderTpl = renderTpl.replace(
          /{{footerButton}}/g,
          RENDERAPI.pickerFooterClearButton(nameOptions)
        );
      } else {
        renderTpl = renderTpl.replace(
          /{{footerButton}}/g,
          RENDERAPI.pickerFooterNowButton(nameOptions)
        );
      }
      var $datePickerHtml = $(
        renderTpl
          .replace(/{{table}}/g, table)
          .replace(/{{year}}/g, dataFormat.year)
          .replace(/{{month}}/g, dataFormat.month)
          .replace("{{sidebar}}", sidebar)
          .replace("{{hasTime}}", hasTime)
          .replace("{{hasSidebar}}", hasSidebar)
      );
    ...
    eventHasTime: function () {
      ...
      this.$container.on("click", ".c-datePicker__input-time", function (
        event
      ) {
        event.stopPropagation();
      });
      ...
      this.$container.on("blur", ".c-datePicker__input-day", function (event) {
        ...
        API.judgeTimeRange(
          _this,
          $(this),
          _this.$container.find(".c-datePicker__input-time")
        );
      });
      // 失焦判断最大值最小值
      this.$container.on("blur", ".c-datePicker__input-time", function (event) {
        ...
        API.judgeTimeRange(
          _this,
          _this.$container.find(".c-datePicker__input-day"),
          $(this)
        );
      });

      // 聚焦时分秒input
      this.$container.on("focus", ".c-datePicker__input-time", function (
        event
      ) {
        event.stopPropagation();
        var _this = API.getPicker($(this));
        ...
        _this.activeTimeWrap = $(this).parents(
    ...
    eventHasTime: function () {
      ...
      this.$container.on("blur", ".c-datePicker__input-day", function (event) {
        ...
        API.judgeTimeRange(
          _this,
          $(this),
          _this.$container.find(".c-datePicker__input-time").eq(index),
          index
        );
      });
      // time失焦判断最大值最小值
      this.$container.on("blur", ".c-datePicker__input-time", function (event) {
        ...
        API.judgeTimeRange(
          _this,
          _this.$container.find(".c-datePicker__input-day").eq(index),
          $(this),
          index
        );
      });
  ...
  $.extend(RangeDatePickerTime.prototype, {
    ...
    initShow: function () {
      ...
      var nameOptions = $.fn.datePicker.dates[this.language];
      var TPL = this.config.isRange
        ? RENDERAPI.rangePickerMainOnlyTimeTpl(nameOptions, hasTime)
        : RENDERAPI.datePickerMainOnlyTimeTpl(hasTime);
  ...
  $.extend(DatePicker.prototype, {
    ...
    event: function () {
      ...
      this.pickerObject.$input.on("blur", function () {
        ...
        if (_this.pickerObject.hasTime) {
          var time = _this.onlyTime
            ? API.timeCheck(valArr[0])
            : valArr[1]
            ? API.timeCheck(valArr[1])
            : false;
          ...
          if (_this.onlyTime) {
            ...
            if (!time || !timeResult) {
              ...
              if (timeResult) {
                time = timeResult[5]
                  ? timeResult[1] +
                    ":" +
                    API.fillTime(timeResult[3]) +
                    ":" +
                    API.fillTime(timeResult[5])
            ...
            if (!time || !timeResult || !dayResult) {
              ...
              if (dayResult) {
                ...
                day =
                  dayResult[1] +
                  _this.pickerObject.splitStr +
                  API.fillTime(dayResult[3]) +
                  _this.pickerObject.splitStr +
                  API.fillTime(dayResult[5]);
              }
              if (timeResult) {
                time = timeResult[5]
                  ? timeResult[1] +
          ...
          if (_this.pickerObject.params.isMonth) {
            var _moment = moment(
              API.newDateFixed(
                _this.pickerObject,
                day + _this.pickerObject.splitStr + "01"
              )
            );
            var result = API.getTimeFormat(_moment);
            var resultJson = API.minMaxFill(
              _this.pickerObject,
              result,
              0,
              "month"
    ...
    betweenHandle: function () {
      ...
      if (_config.between === "month") {
        if (
          beginFormat.year !== endFormat.year ||
          beginFormat.month !== endFormat.month
        ) {
          var val = beginMoment
            .set({ year: endFormat.year, month: endFormat.month - 1, date: 1 })
      ...
      if (_config.between === "year") {
        if (beginFormat.year !== endFormat.year) {
          var val = beginMoment
            .set({ year: endFormat.year, month: 0, date: 1 })
            .format(_config.format);
          this.pickerObject.$inputBegin.val(val);
        }
        return;
